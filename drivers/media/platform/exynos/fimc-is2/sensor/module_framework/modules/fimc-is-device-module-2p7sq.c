...
#ifdef CONFIG_OF
	fimc_is_module_parse_dt(dev, sensor_module_2p7sq_power_setpin);
#endif
...
static const struct of_device_id exynos_fimc_is_sensor_module_2p7sq_match[] = {
	{
...
MODULE_DEVICE_TABLE(of, exynos_fimc_is_sensor_module_2p7sq_match);

static struct platform_driver sensor_module_2p7sq_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-2P7SQ",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_fimc_is_sensor_module_2p7sq_match,
	}
};

static int __init fimc_is_sensor_module_2p7sq_init(void)
{
	int ret;
...
late_initcall(fimc_is_sensor_module_2p7sq_init);
