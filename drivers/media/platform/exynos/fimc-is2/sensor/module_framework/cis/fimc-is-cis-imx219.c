...
#include "fimc-is-cis-imx219-setA.h"
#include "fimc-is-cis-imx219-setB.h"
...
int sensor_imx219_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	ret = fimc_is_sensor_read16(client, 0x0306, &pll_h);
	ret = fimc_is_sensor_read16(client, 0x0307, &pll_l);
	...
	ret = fimc_is_sensor_read16(client, 0x030c, &pll_h);
	ret = fimc_is_sensor_read16(client, 0x030d, &pll_l);
	dbg_sensor(1, "______ secnd_pll_multiplier(%x%x)\n", pll_h, pll_l);
	ret = fimc_is_sensor_read16(client, 0x0160, &pll_h);
	ret = fimc_is_sensor_read16(client, 0x0161, &pll_l);
	dbg_sensor(1, "______ frame_length_lines(%x%x)\n", pll_h, pll_l);
	ret = fimc_is_sensor_read16(client, 0x0162, &pll_h);
	ret = fimc_is_sensor_read16(client, 0x0163, &pll_l);
