...
#include "fimc-is-cis-6b2-setA.h"
#include "fimc-is-cis-6b2-setB.h"
...
static int FPGA_init(struct v4l2_subdev *subdev)
{
...
#endif
	fimc_is_sensor_read16(client, 0x0000, &rev);
	...
	// Color ratio
	fimc_is_sensor_write8(client, 0x31D3, 0x01);    // efuse read en
	fimc_is_sensor_write8(client, 0x3426, 0x3A);    // [4]corr_en, [3:2]gain_b_sel, [1:0]gain_r_sel
	fimc_is_sensor_write8(client, 0x340D, 0x30);    // efuse clock off

	// Analog setting for Normal mode
	fimc_is_sensor_write8(client, 0x3067, 0x25);    // adc_sat[mV]=617mV
	fimc_is_sensor_write8(client, 0x307D, 0x08);    // dbr_tune_tgsl
	fimc_is_sensor_write8(client, 0x307E, 0x08);    // dbr_tune_rg
	fimc_is_sensor_write8(client, 0x307F, 0x08);    // dbr_tune_fdb
	fimc_is_sensor_write8(client, 0x3080, 0x04);    // dbr_tune_ntg
	fimc_is_sensor_write8(client, 0x3073, 0x73);    // comp1_bias, comp2_bias
	fimc_is_sensor_write8(client, 0x3074, 0x45);    // pix_bias, pix_bias_boost
	fimc_is_sensor_write8(client, 0x3075, 0xD4);    // clp_lvl
	fimc_is_sensor_write8(client, 0x3085, 0xF0);    // rdv_option; LOB_PLA enable
	fimc_is_sensor_write8(client, 0x3068, 0x55);    // ms[15:8]; x4~
	fimc_is_sensor_write8(client, 0x3069, 0x00);    // ms[7:0]; x1~x4
	fimc_is_sensor_write8(client, 0x3063, 0x08);    // cds_option[15:8]; [11]ldb nmos sw enable=1
	fimc_is_sensor_write8(client, 0x3064, 0x00);    // cds_option[7:0];
	fimc_is_sensor_write8(client, 0x3010, 0x04);

	// Row-diff improvement
	fimc_is_sensor_write8(client, 0x3247, 0x11); // [4] fadlc_blst_en
	fimc_is_sensor_write8(client, 0x3083, 0x00); // blst_en_cintr = 16
	fimc_is_sensor_write8(client, 0x3084, 0x10);

	// MIPI Clk
	fimc_is_sensor_write8(client, 0x6030, 0x18); // EXTCLK_MHz
	//fimc_is_sensor_write8(client, 0x6030, 0x0C); // EXTCLK_MHz

	//PLL
	fimc_is_sensor_write8(client, 0x0305, 0x06); //pre_pll_clk_div
	fimc_is_sensor_write8(client, 0x0306, 0x00); //pll_multiplier  //200d
	fimc_is_sensor_write8(client, 0x0307, /*140*/0xC8); //pll_multiplier
	fimc_is_sensor_write8(client, 0x3301, 0x03); // PLL_S      //*1/8
	fimc_is_sensor_write8(client, 0x3308, 0x04); //[6]dbr_gclk_auto, [5]always_pllcal_en
	fimc_is_sensor_write8(client, 0x3307, 0x11); //[3:0]div_dbr

	//BIT_RATE_MBPS (100Mbps)
	fimc_is_sensor_write8(client, 0x3351, 0x00);
	fimc_is_sensor_write8(client, 0x3352, 0x64);
	fimc_is_sensor_write8(client, 0x3353, 0x00);
	fimc_is_sensor_write8(client, 0x3354, 0x00);
	fimc_is_sensor_write8(client, 0x7339, 0x03);    // [2]dphy_en1, [1]dphy_en0, [0] dhpy_en_clk

	//Binninf mode Setting
	fimc_is_sensor_write8(client, 0x3190, 0x01);    // hv_avg_mode

	// Output size
	fimc_is_sensor_write8(client, 0x034c, 0x02);    // x_output_size MSB
	fimc_is_sensor_write8(client, 0x034d, 0x80 + cac_margin_x);    // x_output_size LSB
	fimc_is_sensor_write8(client, 0x034e, 0x01);    // y_output_size MSB
	fimc_is_sensor_write8(client, 0x034f, 0xE0 + cac_margin_y);    // y_output_size LSB
	fimc_is_sensor_write8(client, 0x306C, 0x28); // [5]aac_en
	fimc_is_sensor_write8(client, 0x0344, 0x01);    // x_addr_start MSB
	fimc_is_sensor_write8(client, 0x0345, 0x48 - cac_margin_x);    // x_addr_start LSB
	fimc_is_sensor_write8(client, 0x0348, 0x06);    // x_addr_end MSB
	fimc_is_sensor_write8(client, 0x0349, 0x47 + cac_margin_x);    // x_addr_end LSB
	fimc_is_sensor_write8(client, 0x0346, 0x00);    // y_addr_start MSB
	fimc_is_sensor_write8(client, 0x0347, 0x44 - cac_margin_y);    // y_addr_start LSB
	fimc_is_sensor_write8(client, 0x034a, 0x04);    // y_addr_end MSB
	fimc_is_sensor_write8(client, 0x034b, 0x03 + cac_margin_y);    // y_addr_end LSB
	fimc_is_sensor_write8(client, 0x0340, 0x11);    // 1fps
	fimc_is_sensor_write8(client, 0x0341, 0x94);

	/* Pattern selection signal
	 * 0 = No pattern (default) 1 = Solid color 2 = 100 % color bars 3 = Fade to grey' color bars 4 = PN9
	 */
	fimc_is_sensor_write16(client, 0x0600, 0x2);
