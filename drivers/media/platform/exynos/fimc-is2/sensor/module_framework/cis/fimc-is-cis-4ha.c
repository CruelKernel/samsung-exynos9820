...
int sensor_4ha_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = fimc_is_sensor_read8(client, 0x3C02, &data8);
	if (unlikely(!ret)) pr_info("0x3C02(0x%x)\n", data8);
	else goto i2c_err;
	ret = fimc_is_sensor_read8(client, 0x3C03, &data8);
	if (unlikely(!ret)) pr_info("0x3C03(0x%x)\n", data8);
	else goto i2c_err;
	ret = fimc_is_sensor_read8(client, 0x3C05, &data8);
	if (unlikely(!ret)) pr_info("0x3C05(0x%x)\n", data8);
	else goto i2c_err;
	ret = fimc_is_sensor_read8(client, 0x3500, &data8);
	...
	const struct i2c_device_id *id)
{
	...
	for (i = 0; i < sensor_4ha_verify_sensor_mode_size; i++) {
		...
		if (fimc_is_vendor_verify_mipi_channel(sensor_4ha_mipi_sensor_mode[index].mipi_channel,
