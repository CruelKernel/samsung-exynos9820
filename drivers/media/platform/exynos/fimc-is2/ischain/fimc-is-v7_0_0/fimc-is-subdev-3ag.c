...
static int fimc_is_ischain_3ag_cfg(struct fimc_is_subdev *subdev,
...
static int fimc_is_ischain_3ag_start(struct fimc_is_device_ischain *device,
...
static int fimc_is_ischain_3ag_stop(struct fimc_is_device_ischain *device,
	struct fimc_is_subdev *subdev, struct fimc_is_frame *frame,
	u32 *lindex, u32 *hindex, u32 *indexes)
{
	int ret = 0;
...
static int fimc_is_ischain_3ag_tag(struct fimc_is_subdev *subdev,
	...
	struct camera2_node *node)
{
	...
	if (node->request) {
		...
		if (!COMPARE_CROP(otcrop, &otparm) ||
			...
			ret = fimc_is_ischain_3ag_start(device,
				subdev, ldr_frame, queue,
				taa_param, otcrop,
				&lindex, &hindex, &indexes);
			if (ret) {
				merr("fimc_is_ischain_3ag_start is fail(%d)", device, ret);
				goto p_err;
			}
		...
		if (test_bit(FIMC_IS_SUBDEV_RUN, &subdev->state)) {
			ret = fimc_is_ischain_3ag_stop(device,
				subdev, ldr_frame,
				&lindex, &hindex, &indexes);
			if (ret) {
				merr("fimc_is_ischain_3ag_stop is fail(%d)", device, ret);
				goto p_err;
			}
...
const struct fimc_is_subdev_ops fimc_is_subdev_3ag_ops = {
	.bypass			= NULL,
	.cfg			= fimc_is_ischain_3ag_cfg,
	.tag			= fimc_is_ischain_3ag_tag,
};
