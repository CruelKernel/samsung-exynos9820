...
struct fimc_is_clk fimc_is_clk_list[] = {
	REGISTER_CLK("GATE_IS_3AA"),
	REGISTER_CLK("GATE_IS_ISP"),
	REGISTER_CLK("GATE_IS_MCSC"),
	REGISTER_CLK("GATE_IS_VRA"),
	REGISTER_CLK("GATE_IS_TPU"),

	REGISTER_CLK("GATE_IS_CSIS_0"),
	REGISTER_CLK("GATE_IS_CSIS_1"),
	REGISTER_CLK("GATE_IS_CSIS_DMA"),
...
/* for debug */
void __iomem *cmu_cmu;
void __iomem *cmu_is;

static void exynos7872_fimc_is_print_clk_reg(void)
{

}

static int exynos7872_fimc_is_print_clk(struct device *dev)
{
	fimc_is_get_rate(dev, "GATE_IS_3AA");
	fimc_is_get_rate(dev, "GATE_IS_ISP");
	fimc_is_get_rate(dev, "GATE_IS_MCSC");
	fimc_is_get_rate(dev, "GATE_IS_VRA");
	fimc_is_get_rate(dev, "GATE_IS_TPU");

	fimc_is_get_rate(dev, "GATE_IS_CSIS_0");
	fimc_is_get_rate(dev, "GATE_IS_CSIS_1");
	fimc_is_get_rate(dev, "GATE_IS_CSIS_DMA");

	fimc_is_get_rate(dev, "MUX_CIS_CLK0");
	fimc_is_get_rate(dev, "MUX_CIS_CLK1");
	fimc_is_get_rate(dev, "MUX_CIS_CLK2");

	fimc_is_get_rate(dev, "CIS_CLK0");
	fimc_is_get_rate(dev, "CIS_CLK1");
	fimc_is_get_rate(dev, "CIS_CLK2");

	fimc_is_get_rate(dev, "GATE_SMMU_IS");

	exynos7872_fimc_is_print_clk_reg();

	return 0;
}

int exynos7872_fimc_is_clk_gate(u32 clk_gate_id, bool is_on)
{
	return 0;
}

int exynos7872_fimc_is_uart_gate(struct device *dev, bool mask)
{
	return 0;
}

int exynos7872_fimc_is_get_clk(struct device *dev)
{
	const char *name;
	struct clk *clk;
	u32 index;
	...
	/* for debug */
	cmu_cmu = ioremap(0x10560000, SZ_4K);
	cmu_is = ioremap(0x144b0000, SZ_4K);
...
int exynos7872_fimc_is_cfg_clk(struct device *dev)
{
	pr_debug("%s\n", __func__);
...
int exynos7872_fimc_is_clk_on(struct device *dev)
{
	int ret = 0;
	...
	fimc_is_enable(dev, "GATE_IS_3AA");
	fimc_is_enable(dev, "GATE_IS_ISP");
	fimc_is_enable(dev, "GATE_IS_MCSC");
	fimc_is_enable(dev, "GATE_IS_VRA");
	fimc_is_enable(dev, "GATE_IS_TPU");
	...
	fimc_is_enable(dev, "GATE_SMMU_IS");
...
int exynos7872_fimc_is_clk_off(struct device *dev)
{
	int ret = 0;
	struct exynos_platform_fimc_is *pdata;
	...
	fimc_is_disable(dev, "GATE_IS_3AA");
	fimc_is_disable(dev, "GATE_IS_ISP");
	fimc_is_disable(dev, "GATE_IS_MCSC");
	fimc_is_disable(dev, "GATE_IS_VRA");
	fimc_is_disable(dev, "GATE_IS_TPU");
	...
	fimc_is_disable(dev, "GATE_SMMU_IS");
...
{
	exynos7872_fimc_is_clk_gate(clk_gate_id, is_on);
	return 0;
}
