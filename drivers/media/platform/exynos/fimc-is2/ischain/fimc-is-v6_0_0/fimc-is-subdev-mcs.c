	...
	struct camera2_node *node)
{
	...
	unsigned long mcs_input_mode = MCSC_INPUT_FROM_ISP_PATH;
	...
	/* mcsc input select for per-frame setting */
	if (group->head && (group->head->id == GROUP_ID_DCP))
		mcs_input_mode = MCSC_INPUT_FROM_DCP_PATH;
	else
		mcs_input_mode = MCSC_INPUT_FROM_ISP_PATH;

	fimc_is_hw_s_ctrl(NULL, group->id,
		HW_S_CTRL_MCSC_SET_INPUT, (void *)mcs_input_mode);
p_err:
	return ret;
}
