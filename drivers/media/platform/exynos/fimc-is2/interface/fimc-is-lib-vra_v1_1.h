...
#define VRA_DICO_API_VERSION			10
#define VRA_OTF_INPUT_FORMAT			VRA_YUV_FMT_444
#define VRA_TR_HW_RES_PER_FACE			110
#define VRA_FF_HW_RES_PER_LIST			500
#define VRA_IMAGE_SLOTS				5
/* default tune value */
#define VRA_TUNE_TRACKING_MODE			VRA_TRM_FULL_FR_TRACK
#define VRA_TUNE_FACE_PRIORITY			1
#define VRA_TUNE_DISABLE_FRONTAL_ROT_MASK	0x00
#define VRA_TUNE_WORKING_POINT			865
...
enum api_vra_ctrl_task_set_event {
...
enum api_vra_type {
	 ...
	 */
	VRA_ERR_NEW_SENSOR_SIZES		= 0x31,
	/*
	 * The input sizes set for the sensor are different
	 * from maximal sizes defined on initialization -
	 * not allowed when SW padding is used
	 */
...
struct api_vra_alloc_info {
	...
	unsigned char pad_size;			/* HW memory */

	/* 0 stands for no support for 3 planes.*/
	vra_boolean allow_3planes;		/* HW memory */
...
enum api_vra_proc_Type {
	PR_PG,
	/*
	 * 0 - HW PG process - Invoked when called by user.
	 * Notice that "Next" is automatically invoked by HW (when ready).
	 */

	PR_DET,
	/*
	 * 1 - HW Core process "Face detection" -
	 * Invoked after pyramid is ready
	 */

	PR_POST_DET,
	/*
	 * 2 - FW Det results clustering and Merge to output list -
	 * after HW DET faces list is ready.
	 * Optionally - Prepare the input for FF -
	 * list of squares with unique face index per square
	 */

	PR_PRE_HWTR,
	/*
	 * 3 - FW creates the ROIs list for HW tracking and
	 * based on previous results list
	 */

	PR_TR_HW,
	/*
	 * 4 - HW Core process "HW Tracking" - Invoked from Control task -
	 * after TR is finished (might include 0 faces)
	 */

	PR_POST_HWTR,
	/*
	 * 5 - FW create an updated tracking list using the HW Track results.
	 * Optionally - Prepare the input for FF - list of squares with
	 * unique face index per square
	 */

	PR_FF_HW,
	/*
	 * 6 - Optionally - HW Core process Facial features -
	 * Invoked from Control task -
	 * after FF list is ready (and frame is kept)
	 */

	PR_POST_FF,
	/*
	 * 7 - Optionally - Create Facial list and merge to output list -
	 * after HW Facial results list is ready
	 */
