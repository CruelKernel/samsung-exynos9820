...
const struct v4l2_file_operations fimc_is_3xg_video_fops;
const struct v4l2_ioctl_ops fimc_is_3xg_video_ioctl_ops;
const struct vb2_ops fimc_is_3xg_qops;

int fimc_is_30g_video_probe(void *data)
{
	int ret = 0;
	...
	ret = fimc_is_video_probe(video,
		FIMC_IS_VIDEO_3XG_NAME(0),
		FIMC_IS_VIDEO_30G_NUM,
		VFL_DIR_RX,
		&core->resourcemgr.mem,
		&core->v4l2_dev,
		&fimc_is_3xg_video_fops,
		&fimc_is_3xg_video_ioctl_ops);
	if (ret)
...
int fimc_is_31g_video_probe(void *data)
{
	int ret = 0;
	struct fimc_is_core *core;
	struct fimc_is_video *video;
...
static int fimc_is_3xg_video_open(struct file *file)
{
	int ret = 0;
	int ret_err = 0;
	struct fimc_is_video *video;
	...
	ret = open_vctx(file, video, &vctx, device->instance, FRAMEMGR_ID_3XG, name);
	if (ret) {
...
static int fimc_is_3xg_video_close(struct file *file)
{
	int ret = 0;
	int refcount;
...
static unsigned int fimc_is_3xg_video_poll(struct file *file,
...
static int fimc_is_3xg_video_mmap(struct file *file,
...
const struct v4l2_file_operations fimc_is_3xg_video_fops = {
	.owner		= THIS_MODULE,
	.open		= fimc_is_3xg_video_open,
	.release	= fimc_is_3xg_video_close,
	.poll		= fimc_is_3xg_video_poll,
	.unlocked_ioctl	= video_ioctl2,
	.mmap		= fimc_is_3xg_video_mmap,
};
...
static int fimc_is_3xg_video_querycap(struct file *file, void *fh,
...
static int fimc_is_3xg_video_enum_fmt_mplane(struct file *file, void *priv,
...
static int fimc_is_3xg_video_get_format_mplane(struct file *file, void *fh,
...
static int fimc_is_3xg_video_set_format_mplane(struct file *file, void *fh,
	struct v4l2_format *format)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_3xg_video_try_format_mplane(struct file *file, void *fh,
	struct v4l2_format *format)
{
	dbg("%s\n", __func__);
	return 0;
...
static int fimc_is_3xg_video_cropcap(struct file *file, void *fh,
...
static int fimc_is_3xg_video_get_crop(struct file *file, void *fh,
...
static int fimc_is_3xg_video_set_crop(struct file *file, void *fh,
...
static int fimc_is_3xg_video_reqbufs(struct file *file, void *priv,
...
static int fimc_is_3xg_video_querybuf(struct file *file, void *priv,
...
static int fimc_is_3xg_video_qbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_3xg_video_dqbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_3xg_video_prepare(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
p_err:
	minfo("[3%dG:V] %s(%d):%d\n", device, GET_3XG_ID(GET_VIDEO(vctx)), __func__, buf->index, ret);
	return ret;
}

static int fimc_is_3xg_video_streamon(struct file *file, void *priv,
...
static int fimc_is_3xg_video_streamoff(struct file *file, void *priv,
	enum v4l2_buf_type type)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_3xg_video_enum_input(struct file *file, void *priv,
...
static int fimc_is_3xg_video_g_input(struct file *file, void *priv,
	unsigned int *input)
{
	dbg("%s\n", __func__);
	return 0;
...
static int fimc_is_3xg_video_s_input(struct file *file, void *priv,
	unsigned int input)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_3xg_video_g_ctrl(struct file *file, void *priv,
...
static int fimc_is_3xg_video_g_ext_ctrl(struct file *file, void *priv,
	struct v4l2_ext_controls *ctrls)
{
	return 0;
}

static int fimc_is_3xg_video_s_ctrl(struct file *file, void *priv,
	struct v4l2_control *ctrl)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
const struct v4l2_ioctl_ops fimc_is_3xg_video_ioctl_ops = {
	.vidioc_querycap		= fimc_is_3xg_video_querycap,
	.vidioc_enum_fmt_vid_cap_mplane	= fimc_is_3xg_video_enum_fmt_mplane,
	.vidioc_g_fmt_vid_cap_mplane	= fimc_is_3xg_video_get_format_mplane,
	.vidioc_s_fmt_vid_cap_mplane	= fimc_is_3xg_video_set_format_mplane,
	.vidioc_try_fmt_vid_cap_mplane	= fimc_is_3xg_video_try_format_mplane,
	.vidioc_cropcap			= fimc_is_3xg_video_cropcap,
	.vidioc_g_crop			= fimc_is_3xg_video_get_crop,
	.vidioc_s_crop			= fimc_is_3xg_video_set_crop,
	.vidioc_reqbufs			= fimc_is_3xg_video_reqbufs,
	.vidioc_querybuf		= fimc_is_3xg_video_querybuf,
	.vidioc_qbuf			= fimc_is_3xg_video_qbuf,
	.vidioc_dqbuf			= fimc_is_3xg_video_dqbuf,
	.vidioc_prepare_buf		= fimc_is_3xg_video_prepare,
	.vidioc_streamon		= fimc_is_3xg_video_streamon,
	.vidioc_streamoff		= fimc_is_3xg_video_streamoff,
	.vidioc_enum_input		= fimc_is_3xg_video_enum_input,
	.vidioc_g_input			= fimc_is_3xg_video_g_input,
	.vidioc_s_input			= fimc_is_3xg_video_s_input,
	.vidioc_g_ctrl			= fimc_is_3xg_video_g_ctrl,
	.vidioc_s_ctrl			= fimc_is_3xg_video_s_ctrl,
	.vidioc_g_ext_ctrls		= fimc_is_3xg_video_g_ext_ctrl,
};

static int fimc_is_3xg_queue_setup(struct vb2_queue *vbq,
...
static void fimc_is_3xg_stop_streaming(struct vb2_queue *vbq)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = vbq->drv_priv;
	struct fimc_is_queue *queue;
...
static void fimc_is_3xg_buffer_queue(struct vb2_buffer *vb)
{
	int ret = 0;
...
static void fimc_is_3xg_buffer_finish(struct vb2_buffer *vb)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = vb->vb2_queue->drv_priv;
	struct fimc_is_device_ischain *device;
