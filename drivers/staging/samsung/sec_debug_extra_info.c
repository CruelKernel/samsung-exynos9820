...
#include <linux/sec_debug_extra_info_keys.h>
...
void sec_debug_extra_info_key_init(void)
{
	int nr_keys;
...
void sec_debug_extra_info_copy_shared_buffer(bool mflag)
{
	int i;
...
void sec_debug_extra_info_dump_sb_index(void)
{
	int i;
...
void sec_debug_init_extra_info_sbidx(int type, struct sec_debug_sb_index idx, bool mflag)
{
...
void sec_debug_store_extra_info(char (* keys)[MAX_ITEM_KEY_LEN], int nr_keys, char *ptr)
{
	int i;
...
{
	int i, type = SLOT_BK_64;
	unsigned int cnt;
	unsigned long len, max_len, blmark;
	void *p;
	char *v, *start_addr = ptr;
	...
	cnt = sh_buf->sec_debug_sbidx[type].cnt;
	blmark = sh_buf->sec_debug_sbidx[type].blmark;

	pr_crit("%s: cnt: %d, blmark: %lx\n", __func__,
					sh_buf->sec_debug_sbidx[type].cnt, blmark);
	...
	p = get_bk_item_val("ID");
	if (!p) {
		pr_crit("%s: no ID item\n", __func__);
	...
	ptr += snprintf(ptr, MAX_EXTRA_INFO_LEN, "\"%s\":\"%s\"", "ID", (char *)p);
}
