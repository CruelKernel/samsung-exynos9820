		...
		uint32_t mcu_addr, uint32_t x_size, bool start)
{
	...
	writel(mcu_addr + IVA_VMCU_MEM_BASE_ADDR, ch_base + VDMA_CH_CFG1_ADDR);

	/* clear to planar mode w/o flow-control */
	vdma_clear_planar_config(ch_base);

	/* clear the vCF circular buffer RP/WP configuration */
	writel(0x0, ch_base + VDMA_CH_CBRCFG_ADDR(0));
	writel(0x0, ch_base + VDMA_CH_CBWCFG_ADDR(0));
...
static const struct iva_vdma_dump_regs wr_list[] = {
	{ iva_vdma_common_reg,	ARRAY_SIZE(iva_vdma_common_reg) },
	{ iva_vdma_ro_reg,	ARRAY_SIZE(iva_vdma_ro_reg)	},
	{ iva_vdma_wo_reg,	ARRAY_SIZE(iva_vdma_wo_reg)	},
	{ NULL,			0				}
...
static const struct iva_vdma_dump_regs *ch_dump_list[vdma_ch_max] = {
	...
	[vdma_ch5_type_ro_yuv] = ro_yuv_list,
	[vdma_ch6_type_ro_yuv] = ro_yuv_list,
	[vdma_ch7_type_ro_spl] = ro_spl_list,
	...
	[vdma_ch13_type_wo_yuv] = wo_yuv_list,
	[vdma_ch14_type_wo_yuv] = wo_yuv_list,
	[vdma_ch15_type_rw] = wr_list,
};
...
void iva_vdma_show_status(struct iva_dev_data *iva)
{
	...
	vdma_sfr_clk = !!iva_pmu_enable_sfr_clk(iva, pmu_vdma_id, true);
	if (!vdma_sfr_clk)
		dev_info(dev, "vdma sfr clock was off.\n");

	/* GBL */
	dev_info(dev, "------------- VDMA2.0 GBL ------------\n");
	...
	if (!busy_ch_cnt) {
		dev_info(dev, "No busy VDMA2.0 Channels found\n");
		goto out;
	}
...
out:
	if (!vdma_sfr_clk)
		iva_pmu_enable_sfr_clk(iva, pmu_vdma_id, false);
