...
/*
 * ***************** CUSTOMIZATION FILE ********************
 * This file can be edited for your project,
 * and you can customize your transport layer by changing some of the default value
...
#endif
// Defines the number of millisecond before retrying a reliable packet when no acknowledgment is received.
...
// Indicates that this is a reliable packet. Flag detail contains the remote reliable seqId
static const unsigned short FLAG_RELIABLE_PACKET = (1<<1);
// A reliable SeqId was Acked. Flag detail contains the acked reliable seqId
static const unsigned short FLAG_RELIABLE_ACK	= (1<<2);
// A reliable SeqId error was detected. Flag detail contains the last remote reliable seqId
static const unsigned short FLAG_RELIABLE_NACK   = (1<<3);
// Remote PacketLayer detected lost packets (jumps in SeqId). The flag details contains the last remote seqId
static const unsigned short FLAG_MSG_LOST		= (1<<4);
// Garbage bytes detected. The Flag details will contains the number of garbage bytes (capped to 255)
static const unsigned short FLAG_MSG_GARBAGE	 = (1<<5);
// Size of packet will have one byte extension (MSB), contained in the Flags detail
static const unsigned short FLAG_SIZE_EXTENDED   = (1<<6);
// If set, then the flag details will contains a Byte representing the MSB of the 16bit flags
static const unsigned short FLAG_EXTENDED		= (1<<7);
// Packet in that message is internal, and should be processed by the TL
static const unsigned short FLAG_INTERNAL_PACKET = (1<<8);
// Remote side requested to ignore the seqId, i.e. error should not be accounted for in the stats
...
static bool TransportLayer_PacketReceived(void *priv)
{
	...
	/* passed CRC check */
	{
		...
		if (usFlags == 0 && usPayloadSize == usLen) {
			...
			}
			{
				...
				if (bProcessPacket) {
					...
					} else if (!BbdBridge_OnPacketReceived(pucData, usLen)) {
 ...
 *
 */
void bbd_parse_asic_data(unsigned char *pucData, unsigned short usLen,
		...
		case WAIT_FOR_ESC_SOP:
			{
				if (ucData == ESCAPE_CHARACTER) {
					...
					// if we had only one byte, then there is no garbage, any extra is garbage
					m_otCurrentStats.ulRxGarbageBytes += (m_ulByteCntSinceLastValidPacket - 1);
		...
		case WAIT_FOR_EOP:
			{
				if (ucData == EOP_CHARACTER) {
					if (TransportLayer_PacketReceived(priv))
						// we had a valid packet, restart the cnt
					...
					m_uiEscLen = 0;
					// if we had only one byte, then there is no garbage, any extra is garbage
					m_otCurrentStats.ulRxGarbageBytes += (m_ulByteCntSinceLastValidPacket - 2);
...
unsigned char sample1[] = { 0x00, 0x2E, 0xB0, 0x00, 0x85, 0x26, 0x00, 0x21, 0x24, 0x22, 0x00, 0x02, 0x00, 0x1E, 0x00,
		0x37, 0x0E, 0xD3, 0x00, 0xCB, 0xFF, 0x50, 0x00, 0xDE, 0x00, 0x56, 0x00, 0xC7, 0xFF, 0x02, 0x00,
		0x00, 0x00, 0x37, 0x00, 0x90, 0xFE, 0x18, 0xFF, 0xB7, 0x46, 0x02, 0x00, 0x00, 0x00, 0xB2, 0xB0, 0x01};
