 ...
 *
 * $Id: dhd_linux_pktdump.c 788969 2018-11-14 14:04:24Z $
 */
...
/* Debug prints */
#define DBGREPLAY		"Replay: %02x%02x%02x%02x%02x%02x%02x%02x"
...
#define EAP_PRINT(str) \
	DHD_PKTDUMP(("ETHER_TYPE_802_1X[%s] [%s]: " str "\n", \
	ifname, tx ? "TX" : "RX"))
	...
	uint32 pktlen, bool tx)
{
	...
	if (eap_wsc->opcode == WSC_OPCODE_MSG) {
		...
		if (msg && msglen) {
			switch (*msg) {
			case WSC_MSG_M1:
				DHD_STATLOG_DATA(dhd, ST(WPS_M1), ifidx, tx);
				EAP_PRINT("EAP Packet, WPS M1");
				break;
			case WSC_MSG_M2:
				DHD_STATLOG_DATA(dhd, ST(WPS_M2), ifidx, tx);
				EAP_PRINT("EAP Packet, WPS M2");
				break;
			case WSC_MSG_M3:
				DHD_STATLOG_DATA(dhd, ST(WPS_M3), ifidx, tx);
				EAP_PRINT("EAP Packet, WPS M3");
				break;
			case WSC_MSG_M4:
				DHD_STATLOG_DATA(dhd, ST(WPS_M4), ifidx, tx);
				EAP_PRINT("EAP Packet, WPS M4");
				break;
			case WSC_MSG_M5:
				DHD_STATLOG_DATA(dhd, ST(WPS_M5), ifidx, tx);
				EAP_PRINT("EAP Packet, WPS M5");
				break;
			case WSC_MSG_M6:
				DHD_STATLOG_DATA(dhd, ST(WPS_M6), ifidx, tx);
				EAP_PRINT("EAP Packet, WPS M6");
				break;
			case WSC_MSG_M7:
				DHD_STATLOG_DATA(dhd, ST(WPS_M7), ifidx, tx);
				EAP_PRINT("EAP Packet, WPS M7");
				break;
			case WSC_MSG_M8:
				DHD_STATLOG_DATA(dhd, ST(WPS_M8), ifidx, tx);
		...
		DHD_STATLOG_DATA(dhd, ST(WSC_START), ifidx, tx);
		EAP_PRINT("EAP Packet, WSC Start");
	} else if (eap_wsc->opcode == WSC_OPCODE_DONE) {
		DHD_STATLOG_DATA(dhd, ST(WSC_DONE), ifidx, tx);
	...
	uint32 pktlen, bool tx)
{
	...
	if (eap_hdr->code == EAP_CODE_REQUEST ||
		...
		case EAP_TYPE_IDENT:
			if (isreq) {
				DHD_STATLOG_DATA(dhd, ST(EAP_REQ_IDENTITY), ifidx, tx);
				EAP_PRINT("EAP Packet, Request, Identity");
			} else {
				DHD_STATLOG_DATA(dhd, ST(EAP_RESP_IDENTITY), ifidx, tx);
				EAP_PRINT("EAP Packet, Response, Identity");
			}
			break;
		case EAP_TYPE_TLS:
			if (isreq) {
				DHD_STATLOG_DATA(dhd, ST(EAP_REQ_TLS), ifidx, tx);
				EAP_PRINT("EAP Packet, Request, TLS");
			} else {
				DHD_STATLOG_DATA(dhd, ST(EAP_RESP_TLS), ifidx, tx);
				EAP_PRINT("EAP Packet, Response, TLS");
			}
			break;
		case EAP_TYPE_LEAP:
			if (isreq) {
				DHD_STATLOG_DATA(dhd, ST(EAP_REQ_LEAP), ifidx, tx);
				EAP_PRINT("EAP Packet, Request, LEAP");
			} else {
				DHD_STATLOG_DATA(dhd, ST(EAP_RESP_LEAP), ifidx, tx);
				EAP_PRINT("EAP Packet, Response, LEAP");
			}
			break;
		case EAP_TYPE_TTLS:
			if (isreq) {
				DHD_STATLOG_DATA(dhd, ST(EAP_REQ_TTLS), ifidx, tx);
				EAP_PRINT("EAP Packet, Request, TTLS");
			} else {
				DHD_STATLOG_DATA(dhd, ST(EAP_RESP_TTLS), ifidx, tx);
				EAP_PRINT("EAP Packet, Response, TTLS");
			}
			break;
		case EAP_TYPE_AKA:
			if (isreq) {
				DHD_STATLOG_DATA(dhd, ST(EAP_REQ_AKA), ifidx, tx);
				EAP_PRINT("EAP Packet, Request, AKA");
			} else {
				DHD_STATLOG_DATA(dhd, ST(EAP_RESP_AKA), ifidx, tx);
				EAP_PRINT("EAP Packet, Response, AKA");
			}
			break;
		case EAP_TYPE_PEAP:
			if (isreq) {
				DHD_STATLOG_DATA(dhd, ST(EAP_REQ_PEAP), ifidx, tx);
				EAP_PRINT("EAP Packet, Request, PEAP");
			} else {
				DHD_STATLOG_DATA(dhd, ST(EAP_RESP_PEAP), ifidx, tx);
				EAP_PRINT("EAP Packet, Response, PEAP");
			}
			break;
		case EAP_TYPE_FAST:
			if (isreq) {
				DHD_STATLOG_DATA(dhd, ST(EAP_REQ_FAST), ifidx, tx);
				EAP_PRINT("EAP Packet, Request, FAST");
			} else {
				DHD_STATLOG_DATA(dhd, ST(EAP_RESP_FAST), ifidx, tx);
				EAP_PRINT("EAP Packet, Response, FAST");
			}
			break;
		case EAP_TYPE_PSK:
			if (isreq) {
				DHD_STATLOG_DATA(dhd, ST(EAP_REQ_PSK), ifidx, tx);
				EAP_PRINT("EAP Packet, Request, PSK");
			} else {
				DHD_STATLOG_DATA(dhd, ST(EAP_RESP_PSK), ifidx, tx);
				EAP_PRINT("EAP Packet, Response, PSK");
			}
			break;
		case EAP_TYPE_AKAP:
			if (isreq) {
				DHD_STATLOG_DATA(dhd, ST(EAP_REQ_AKAP), ifidx, tx);
				EAP_PRINT("EAP Packet, Request, AKAP");
			} else {
				DHD_STATLOG_DATA(dhd, ST(EAP_RESP_AKAP), ifidx, tx);
		...
		DHD_STATLOG_DATA(dhd, ST(EAP_SUCCESS), ifidx, tx);
		EAP_PRINT("EAP Packet, Success");
	} else if (eap_hdr->code == EAP_CODE_FAILURE) {
		DHD_STATLOG_DATA(dhd, ST(EAP_FAILURE), ifidx, tx);
...
dhd_dump_eapol_4way_message(dhd_pub_t *dhd, int ifidx, uint8 *pktdata, bool tx)
{
	...
	if (eap_key->type != EAPOL_WPA2_KEY) {
		DHD_PKTDUMP(("ETHER_TYPE_802_1X[%s] [%s]: ver %d type %d "
			"keytype %d keyinfo 0x%02X\n", ifname, tx ? "TX" : "RX",
	...
	switch (type) {
	case EAPOL_4WAY_M1:
		DHD_STATLOG_DATA(dhd, ST(EAPOL_M1), ifidx, tx);
		EAP_PRINT("EAPOL Packet, 4-way handshake, M1");
		break;
	case EAPOL_4WAY_M2:
		DHD_STATLOG_DATA(dhd, ST(EAPOL_M2), ifidx, tx);
		EAP_PRINT("EAPOL Packet, 4-way handshake, M2");
		break;
	case EAPOL_4WAY_M3:
		DHD_STATLOG_DATA(dhd, ST(EAPOL_M3), ifidx, tx);
		EAP_PRINT("EAPOL Packet, 4-way handshake, M3");
		break;
	case EAPOL_4WAY_M4:
		DHD_STATLOG_DATA(dhd, ST(EAPOL_M4), ifidx, tx);
		EAP_PRINT("EAPOL Packet, 4-way handshake, M4");
		break;
	case EAPOL_GROUPKEY_M1:
		DHD_STATLOG_DATA(dhd, ST(EAPOL_GROUPKEY_M1), ifidx, tx);
		DHD_PKTDUMP(("ETHER_TYPE_802_1X[%s] [%s]: EAPOL Packet, "
			"GROUP Key handshake, M1 " DBGREPLAY "\n", ifname,
			tx ? "TX" : "RX", REPLAY_FMT(eap_key)));
		break;
	case EAPOL_GROUPKEY_M2:
		DHD_STATLOG_DATA(dhd, ST(EAPOL_GROUPKEY_M2), ifidx, tx);
		DHD_PKTDUMP(("ETHER_TYPE_802_1X[%s] [%s]: EAPOL Packet, "
			"GROUP Key handshake, M2 " DBGREPLAY "\n", ifname,
			tx ? "TX" : "RX", REPLAY_FMT(eap_key)));
		break;
	default:
		DHD_STATLOG_DATA(dhd, ST(8021X_OTHER), ifidx, tx);
		DHD_PKTDUMP(("ETHER_TYPE_802_1X[%s] [%s]: ver %d type %d "
			"keytype %d keyinfo 0x%02X\n", ifname, tx ? "TX" : "RX",
			eapol_hdr->version, eapol_hdr->type, eap_key->type,
			(uint32)hton16(eap_key->key_info)));
	...
	uint32 pktlen, bool tx)
{
	...
	if (eapol_hdr->type == EAP_PACKET) {
		...
		DHD_STATLOG_DATA(dhd, ST(EAPOL_START), ifidx, tx);
		...
		DHD_STATLOG_DATA(dhd, ST(8021X_OTHER), ifidx, tx);
		DHD_PKTDUMP(("ETHER_TYPE_802_1X[%s] [%s]: ver %d, type %d\n",
			ifname, tx ? "TX" : "RX", eapol_hdr->version,
			eapol_hdr->type));
	}
}
...
dhd_icmp_dump(dhd_pub_t *dhdp, int ifidx, uint8 *pktdata, bool tx)
{
	...
	if (icmph->type == ICMP_TYPE_ECHO_REQUEST) {
		DHD_STATLOG_DATA(dhdp, ST(ICMP_PING_REQ), ifidx, tx);
		DHD_PKTDUMP_MEM(("PING REQUEST[%s] [%s] : SEQNUM=%d\n",
			ifname, tx ? "TX" : "RX", ntoh16(ICMP_ECHO_SEQ(icmph))));
	} else if (icmph->type == ICMP_TYPE_ECHO_REPLY) {
		DHD_STATLOG_DATA(dhdp, ST(ICMP_PING_RESP), ifidx, tx);
		DHD_PKTDUMP_MEM(("PING REPLY[%s] [%s] : SEQNUM=%d\n",
			ifname, tx ? "TX" : "RX", ntoh16(ICMP_ECHO_SEQ(icmph))));
	} else if (icmph->type == ICMP_TYPE_DEST_UNREACH) {
		DHD_STATLOG_DATA(dhdp, ST(ICMP_DEST_UNREACH), ifidx, tx);
		DHD_PKTDUMP_MEM(("ICMP DEST UNREACH[%s] [%s] : CODE=%d\n",
			ifname, tx ? "TX" : "RX", icmph->code));
	} else {
		DHD_STATLOG_DATA(dhdp, ST(ICMP_OTHER), ifidx, tx);
		DHD_PKTDUMP_MEM(("ICMP [%s] [%s] : TYPE=%d, CODE=%d\n",
...
dhd_arp_dump(dhd_pub_t *dhdp, int ifidx, uint8 *pktdata, bool tx)
{
	...
	ifname = dhd_ifname(dhdp, ifidx);
	if (arph->oper == hton16(ARP_OPC_REQUEST)) {
		DHD_STATLOG_DATA(dhdp, ST(ARP_REQ), ifidx, tx);
		DHD_PKTDUMP_MEM(("ARP REQUEST[%s] [%s]\n", ifname,
			tx ? "TX" : "RX"));
	} else if (arph->oper == hton16(ARP_OPC_REPLY)) {
		DHD_STATLOG_DATA(dhdp, ST(ARP_RESP), ifidx, tx);
		DHD_PKTDUMP_MEM(("ARP RESPONSE[%s] [%s]\n", ifname,
			tx ? "TX" : "RX"));
	} else {
		DHD_PKTDUMP_MEM(("ARP [%s] [%s] op_code=%d\n", ifname,
			tx ? "TX" : "RX", ntoh16(arph->oper)));
	}
}
#endif /* DHD_ARP_DUMP */
...
dhd_dns_dump(dhd_pub_t *dhdp, int ifidx, uint8 *pktdata, bool tx)
{
	...
	if (GET_DNS_QR(flags)) {
		/* Response */
		DHD_STATLOG_DATA(dhdp, ST(DNS_RESP), ifidx, tx);
		DHD_PKTDUMP_MEM(("DNS RESPONSE[%s] [%s] ID:0x%04X, OPCODE:%s, RCODE:%d\n",
			ifname, tx ? "TX" : "RX", (uint32)(htons(dnsh->id)),
			DNS_UNASSIGNED_OPCODE(flags) ? "UNASSIGNED" :
			dns_opcode_types[opcode], GET_DNS_RCODE(flags)));
	} else {
		/* Request */
		DHD_STATLOG_DATA(dhdp, ST(DNS_QUERY), ifidx, tx);
		DHD_PKTDUMP_MEM(("DNS REQUEST[%s] [%s] ID:0x%04X, OPCODE:%s\n",
			ifname, tx ? "TX" : "RX", (uint32)(hton16(dnsh->id)),
			DNS_UNASSIGNED_OPCODE(flags) ? "UNASSIGNED" :
			dns_opcode_types[opcode]));
	}
}
#endif /* DHD_DNS_DUMP */
