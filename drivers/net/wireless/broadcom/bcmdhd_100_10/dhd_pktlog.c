...
dhd_pktlog_filter_pull_forward(dhd_pktlog_filter_t *filter, uint32 del_filter_id, uint32 list_cnt)
{
	...
	if (del_filter_id < list_cnt) {
		move_size = sizeof(dhd_pktlog_filter_info_t);
		/* if filter is not end of list, pull forward filter list after removing it */
		for (pos = del_filter_id-1; pos < list_cnt -1; pos++) {
			filter->info[pos+1].id -= 1;
			memcpy_s(&filter->info[pos], move_size,
					&filter->info[pos+1], move_size);
		}
