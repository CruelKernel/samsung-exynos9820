...
DHDCFLAGS += -DWL_NAN -DWL_NAN_DISC_CACHE -DWL_NAN_ENABLE_MERGE
#endif // endif
...
# Add check delay 4-Way M4 ack
DHDCFLAGS += -DDHD_CHECK_4WAY_M4ACKED
...
#  DHDCFLAGS += -DWLADPS_SEAK_AP_WAR
  ...
  DHDCFLAGS += -DWLADPS_ENERGY_GAIN
  DHDCFLAGS += -DADPS_GAIN_2G_PM0_IDLE=0
  DHDCFLAGS += -DADPS_GAIN_2G_TX_PSPOLL=0
  DHDCFLAGS += -DADPS_GAIN_5G_PM0_IDLE=1800
  DHDCFLAGS += -DADPS_GAIN_5G_TX_PSPOLL=250
  DHDCFLAGS += -DWL_BAM
...
# LOGTRACE_EVENT
  ...
  DHDCFLAGS += -DCUSTOM_CONTROL_LOGTRACE
# LAST ROAM EVENT LOG
...
# SoftAP BW control for power save
  DHDCFLAGS += -DSUPPORT_AP_BWCTRL
# eLNA bypass control in SoftAP mode
  DHDCFLAGS += -DSUPPORT_SOFTAP_ELNA_BYPASS
  DHDCFLAGS += -DWL_SUPP_EVENT
  DHDCFLAGS += -DFILTER_IE
  DHDCFLAGS += -DNDO_CONFIG_SUPPORT
...
# Update WIPS event to framework
  DHDCFLAGS += -DWL_WIPSEVT
  DHDCFLAGS += -DWL_DISABLE_EVENT_ECNT
endif
