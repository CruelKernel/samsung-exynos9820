...
void mcd_cm_sfr(struct mcd_hdr_device *hdr, const struct mcd_cm_params_info *params)
{
	...
	if ((src_gamma == INDEX_GAMMA_UNSPECIFIED) || (dst_gamma == INDEX_GAMMA_UNSPECIFIED) ||
...

		{
			...
			const unsigned int valid_hdr10p = ((valid_hdr10 == 1) && (params->hdr10p_lut != NULL)) ? (((*params->hdr10p_lut) == 1) ? 1 : 0) : 0;
			unsigned int applied_max_luminance = params->dst_max_luminance;

			if ((hdr->id == MCD_VGRFS) && (valid_hdr10p == 1) && (o_type == INDEX_TYPE_SDR)) {
				is_hdr10p = 1;
				table_dm = params->hdr10p_lut + 2;
				applied_max_luminance = *(params->hdr10p_lut + 1);
			}
			//-------------------------------------------------------------------------------------------------
			{
				const unsigned int dither_allow = ((is_hdr10p == 0) && (valid_hdr10 == 1) && (params->src_max_luminance > 3000)) ? 1 : 0;
