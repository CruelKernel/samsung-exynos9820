...
static struct link_device *g_ld;
#endif
...
#ifdef CONFIG_SEC_SIPC_DUAL_MODEM_IF
bool shmem_rild_ready(void)
{
	if (g_ld)
		return rild_ready(g_ld);
	else
		return false;
}
#endif
		...
		struct io_device *iod, unsigned long arg)
{
	...
	err = exynos_smc(SMC_ID, (SMC_MODE_SET_CP2CP_MEM | cp2cp_base),
			cp2cp_size, (CP_BASE_ADDR + cp2cp_offset));
...
static int shmem_force_dump(struct link_device *ld, struct io_device *iod)
{
	...
	mif_err("+++\n");
	if (ld->crash_type && ld->crash_type <= CRASH_REASON_USER) {
...
#if defined(CONFIG_SEC_SIPC_DUAL_MODEM_IF)
static void shmem_cp2ap_change_ul_path_handler(void *data)
{
	struct mem_link_device *mld = (struct mem_link_device *)data;
...
#if defined(CONFIG_SEC_SIPC_DUAL_MODEM_IF)
	memset(&(map->ulpath), 0, sizeof(struct shmem_ulpath_table));
	map->ulpath.magic = SHM_2CP_UL_PATH_CTL_MAGIC;
#endif
}
		...
		const char *buf, size_t count)
{
	...
	if (unlikely(!cp_online(mc)))
		...
		if (index < RMNET_COUNT) {
			map->ulpath.ctl[index].status = status;
			map->ulpath.ctl[index].path = path;
			update_ul_path_table(mld);
		}
	}
...
#ifdef CONFIG_CP_PKTPROC
	err = pktproc_create(pdev, mld, shm_get_pktproc_base(), shm_get_pktproc_size());
	if (err < 0) {
