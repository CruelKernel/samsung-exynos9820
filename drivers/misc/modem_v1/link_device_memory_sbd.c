			   ...
			   u8 *shmem_base, unsigned int shmem_size)
{
	...
	sl->shmem_size = shmem_size + 0x4900000 + shm_get_pktproc_desc_size() + shm_get_pktproc_data_size();
#else
	sl->shmem_size = shmem_size + shm_get_pktproc_desc_size() + shm_get_pktproc_data_size();
#endif
#else
#ifdef CONFIG_CP_LINEAR_WA
	/*
...
#if defined(CONFIG_SEC_MODEM_S5100)
		volatile unsigned int len_ch;

		/* It seems CP see ch_id before ch_id isn't written.
		 * write ch & len at the same time temporarily. */
		len_ch = (skb->len & 0xFFFF) | (ch << 16);
		rb->size_v[in] = len_ch;
#else
