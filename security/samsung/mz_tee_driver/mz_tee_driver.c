...
/////////////////////////////////////////// TA message struct
#define MAX_SHMEM_LEN_BYTES (20 * 1024)
...
struct tci_msg {
	uint32_t cmd;
	uint32_t pid;
	uint8_t encrypt[AES_BLOCK_SIZE_FOR_TZ];
		...
		uint16_t encrypt_len)
{
	...
	mzbuffer->pid = tgid;
	if (encrypt != NULL) {
		memcpy(mzbuffer->encrypt, encrypt, encrypt_len);
		mzbuffer->encrypt_len = encrypt_len;
	}
	...
	if (encrypt != NULL) {
		encrypt_len = mzbuffer->encrypt_len;
		memcpy(encrypt, mzbuffer->encrypt, encrypt_len);
	}
...
static int send_cmd_kthread(uint16_t cmd, uint32_t tgid, u8 *encrypt, uint16_t encrypt_len)
{
	...
	cmd_msg.encrypt_len = encrypt_len;
	cmd_msg.rc = -EBADMSG;
...
MzResult getkeyt(uint32_t tgid, u8 *key, uint16_t key_len)
{
	MzResult mzret = MZ_SUCCESS;
	int taret;

	taret = send_cmd_kthread(CMD_GETKEY, tgid, key, key_len);
...
MzResult removekeyt(uint32_t tgid)
{
	MzResult mzret = MZ_SUCCESS;
	int taret;

	taret = send_cmd_kthread(CMD_REMOVEKEY, tgid, NULL, 0);
	if (taret != MZ_TA_SUCCESS) {
		pr_err("MZ %s ta return error %d\n", __func__, taret);
		mzret = MZ_TA_FAIL;
	}
